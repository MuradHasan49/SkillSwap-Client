...
export const metadata = { title: "My Transactions - SkillSwap" };

export default async function ClientTransactionsPage() {
  const user = await requireRole("client");
  const db = await getDb();
  
  const payments = await db.collection("payments").find({ client_email: user.email }).sort({ paid_at: -1 }).toArray();

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Payment History</h2>
        <p className="text-gray-500 dark:text-gray-400">View your transaction history and past payments.</p>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-gray-50 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700">
                <tr>
                  <th className="px-6 py-4 font-semibold text-gray-900 dark:text-white">Transaction ID</th>
